...
void recorrer_Nivel_x(int *indicex, int *indicesecond, int *eof, int nivelx, int caso, int primerBL, unsigned int nivel_punteros, unsigned int (*bloques_punteros)[NPUNTEROS], int *contador_breads, int *bloque_modificado, int *indices_primerBL)
{
    ...
    if (caso == 1)
    {
        bloque_modificado[nivel_punteros - nivelx - 1] = 0;
    }
    
    (*contador_breads)++;
    if (*indicex == indices_primerBL[nivel_punteros - nivelx])
    {
        *indicesecond = obtener_indice(primerBL, nivel_punteros - nivelx);
        indices_primerBL[nivel_punteros - nivelx - 1] = *indicesecond;
    }
    else
        (*indicesecond) = 0;
...
void recorrid_nivelIndirectos(int *eof, unsigned int ultimoBL, unsigned int (*bloques_punteros)[NPUNTEROS], unsigned int nivel_punteros, int *BLliberado, int *bloque_modificado, int nivelx, int *indicex, unsigned int nblog, int *liberados)
{
    while (!(*eof) && (*indicex) < NPUNTEROS)
    {
        if (nblog == ultimoBL)
            *eof = 1;
        if (bloques_punteros[nivel_punteros - nivelx][*indicex])
        {
            liberar_bloque(bloques_punteros[nivel_punteros - nivelx][*indicex]);
#if DEBUGN6
            fprintf(stderr, "[liberar_bloques_inodo()→ liberado BF %d de datos para BL %d]\n", bloques_punteros[nivel_punteros - nivelx][*indicex], nblog);
            *BLliberado = nblog;
#endif
            (*liberados)++;
            bloques_punteros[nivel_punteros - nivelx][*indicex] = 0;
            bloque_modificado[nivel_punteros - nivelx] = 1;
        }
        (*indicex)++;
    }
}

int memAuxiliarcmp(unsigned int (*bloques_punteros)[NPUNTEROS], unsigned char *bufAux_punteros, struct inodo *inodo, int *contador_bwrites, int *liberados, int *bloque_modificado, int nivelx, unsigned int nivel_punteros, int caso, int *indice, int *BLliberado)
{
    int liberar = -1;
    if (caso == 1)
    {

        liberar = inodo->punterosIndirectos[nivelx - 1];
    }
    else
    {
        liberar = bloques_punteros[nivel_punteros - 1][*indice];
    }

    if (memcmp(bloques_punteros[nivel_punteros - nivelx], bufAux_punteros, BLOCKSIZE) == 0)
    {
        liberar_bloque(liberar); // de punteros
#if DEBUGN6
        fprintf(stderr, "[liberar_bloques_inodo()→ liberado BF %d de punteros_nivel%d correspondiente al BL %d]\n", inodo->punterosIndirectos[nivelx], nivel_punteros, *BLliberado);
#endif
        (*liberados)++;

        // aqui depende del nivel
        if (caso == 1)
        {
            inodo->punterosIndirectos[nivelx - 1] = 0;
        }
        else
        {
            bloques_punteros[nivel_punteros - nivelx + 1][*indice] = 0;
            bloque_modificado[nivel_punteros - nivelx + 1] = 1;
        }
    }
    else
    { // escribimos en el dispositivo el bloque de punteros, si ha sido modificado
        if (bloque_modificado[nivel_punteros - nivelx])
        {
            // aqui tambien el arreglo cambia
            if (bwrite(liberar, bloques_punteros[nivel_punteros - nivelx]) < 0)
                return -1;
            (*contador_bwrites)++;
        }
    }
...
// BLliberado=nblog;
    ...
    if (primerBL < INDIRECTOS0 && !eof)
    {
        ...
        if (inodo->punterosIndirectos[0])
        {
            ...
            recorrid_nivelIndirectos(&eof, ultimoBL, bloques_punteros, nivel_punteros, &BLliberado, bloque_modificado, 1, &i, DIRECTOS + i, &liberados);

            if (memAuxiliarcmp(bloques_punteros, bufAux_punteros, inodo, &contador_bwrites, &liberados, bloque_modificado, 1, nivel_punteros, 1, &i, &BLliberado) == FALLO)
            {
                perror("Error memcmp()");
                return FALLO;
            }
        ...
        if (inodo->punterosIndirectos[1])
        {
            ...
            while (!eof && i < NPUNTEROS)
            {
                if (bloques_punteros[nivel_punteros - 1][i])
                {
                   ...
                   recorrer_Nivel_x( &i, &j, &eof ,  1,  1,primerBL, nivel_punteros, bloques_punteros,&contador_breads, bloque_modificado, indices_primerBL);


                    recorrid_nivelIndirectos(&eof, ultimoBL, bloques_punteros, nivel_punteros, &BLliberado, bloque_modificado, 2, &j, INDIRECTOS0 + i * NPUNTEROS + j, &liberados);

                    if (memAuxiliarcmp(bloques_punteros, bufAux_punteros, inodo, &contador_bwrites, &liberados, bloque_modificado, 2, nivel_punteros, 2, &i, &BLliberado) == FALLO)
                    {
                        perror("Error memcmp()");
                        return FALLO;
                    }
        ...
        if (inodo->punterosIndirectos[2])
        {
            ...
            while (!eof && i < NPUNTEROS)
            {
                if (bloques_punteros[nivel_punteros - 1][i])
                {
                     ...
                     recorrer_Nivel_x( &i, &j,  &eof ,  1,  0, primerBL, nivel_punteros, bloques_punteros,&contador_breads, bloque_modificado, indices_primerBL);
                    ...
                    while (!eof && j < NPUNTEROS)
                    {
                        if (bloques_punteros[nivel_punteros - 2][j])
                        {
                            ...
                            recorrer_Nivel_x( &j, &k,  &eof ,  2,  0, primerBL, nivel_punteros, bloques_punteros, &contador_breads, bloque_modificado, indices_primerBL);

                            recorrid_nivelIndirectos(&eof, ultimoBL, bloques_punteros, nivel_punteros, &BLliberado, bloque_modificado, 3, &k, INDIRECTOS1 + i * NPUNTEROS2 + j * NPUNTEROS + k, &liberados);

                            if (memAuxiliarcmp(bloques_punteros, bufAux_punteros, inodo, &contador_bwrites, &liberados, bloque_modificado, 3, nivel_punteros, 2, &j, &BLliberado) == FALLO)
                            {
                                perror("Error memcmp()");
                                return FALLO;
                            }
